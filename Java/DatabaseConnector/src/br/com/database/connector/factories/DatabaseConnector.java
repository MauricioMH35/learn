...
public class DatabaseConnector implements IDatabase {
    ...
    private Properties prop = DatabaseProperties.get().prop("default");
    ...
    public Connection connect() {
        try {
            ...
            status = "[database] conexão estabelecida com sucesso!";

        } catch(ClassNotFoundException e) {
            status = "[database] não foi possivel operar com o módulo de conexão!";
            e.printStackTrace();

        } catch(SQLException e) {
            status = "[database] não foi possivel estabelecer conexão!";
            e.printStackTrace();
    ...
    public boolean close() {
        try {
            if(connection != null) {
              ...
              status = "[database] a conexão não foi iniciada, sendo assim o processo para encerra-la não foi " +
                      "executado!";

            }

        } catch(SQLException e) {
            status = "[database] não foi possicel encerrar a conexão!";
            e.printStackTrace();
    ...
    public Connection restart() {
        ...
        if(connection != null)
            status = "[database] conexão reiniciada com sucesso!";
